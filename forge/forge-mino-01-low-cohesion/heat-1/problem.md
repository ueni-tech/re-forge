...
- `new Price(amount)`: 負の金額は例外を投げる。0 は許容する
- `withTax(rate)`: 元の `Price` は変更せず、新しい `Price` を返す。負の税率は例外を投げる
- `discount(rate)`: 元の `Price` は変更せず、新しい `Price` を返す。割引率は \([0, 1]\) の範囲外なら例外を投げる
- 端数処理は `Math.floor`（切り捨て）
- `toDisplay()`: `¥1,200` のような日本円表記を返す
- `toNumber()`: 内部の amount を返す（エスケープハッチ）
