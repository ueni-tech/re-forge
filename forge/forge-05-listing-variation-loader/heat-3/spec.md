...
`problem.md` の4問（正常時 / 困った入力 / しないこと / 暗黙の決め）それぞれについて、【契約】に書けていたか確認する。

- **正常時**: 成功時の `PrepareResult` を1行で書けたか
- **困った入力**: 空 SKU / `getParam` が null / 無効 code を契約に書けたか（DI の理由と混ぜていないか）
- **しないこと**: 例外を投げない、`window` を触らない、を書けたか
- **暗黙の決め**: `null` vs 空 `PrepareResult`、早期 return の位置、を書けたか
