...
実務では「どの type を対象にするか」「未チェックや空文字をどう扱うか」は業務側と合意して仕様化する。
この re-forge では、その**合意済み仕様は problem.md に明記**する（一次ソース）。

- `el.type` が `"radio"` または `"checkbox"` で `el.checked === false` のとき、そのエントリはスキップする
- `el.dataset.basketParam` が `undefined` または `""` のとき、そのエントリはスキップする
- 採用する値:
  - `el.dataset.basketValue` が `undefined` でも `""` でもなければ `basketValue` を採用する
  - そうでなければ `el.value` を採用する（フォールバック）
- `type` が `"text"` などラジオ/チェックボックス以外のとき、`checked` の値に関係なく上記の判定だけで採用可否を決める
